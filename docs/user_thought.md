...
## 34. Dense Edge Divider Routing Decision

Decision:

```text
- Dense fan-out and fan-in connector clusters may use layout-only routing dividers.
- A divider is generated by the layout engine, not by Mermaid parsing, when more than four relationships compete for the same endpoint direction.
- For fan-out, targets are bucketed by `common source node + remote group`.
...
- hardValid is false when there are node overlaps, group overlaps, edge-node hits, divider-node hits, endpoint-divider interior hits, displayed segment overlaps, edge identity violations, invalid dividers, or routing fallbacks.
- Segment overlap metrics are split into all segmentOverlaps and hard-failure illegalSegmentOverlaps.
...
## 37. Routing V2 Slice 5A Strict Segment Sharing Decision

Decision:

```text
- edgeCrossings remains a soft quality metric and never affects hardValid.
- edgeNodeHits, routing fallback/failure, invalid dividers, edge identity violations, and illegalSegmentOverlaps are hard failures.
- Ordinary routed edges may not share segments, including same-source or same-target edges.
- Dividers are the only bundling mechanism, but the engine must materialize one physical trunk plus spokes before final validation.
- There is no final segment-overlap exemption: duplicated divider trunks, divider leaves, ordinary edges, invalid dividers, and same-source/same-target routes may not overlap.
- LayoutRunReport.diagnostics and DiagramDocument.layout.diagnostics carry structured diagnostics.
- DiagramDocument.diagnostics remains a human-readable message stream.
...
## 39. Standalone Python Routing Extraction Decision

Decision:

```text
- scripts/autodiagram_standalone.py is an extraction utility for portable/offline routing-v2 generation, not a new backend.
- The product architecture remains structured input -> DiagramDocument -> layout intent -> automatic placement/routing -> mxGraph XML export.
- The TypeScript packages remain the shared product implementation for CLI, web, and future desktop workflows.
- The Python utility is stdlib-only and v2-only; legacy routing is intentionally rejected.
- The Python utility may target behavioral parity with current routing-v2 tests rather than byte-for-byte XML parity.
