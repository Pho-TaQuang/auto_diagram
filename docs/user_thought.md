...
- For fan-out, all target groups or target nodes are treated as one cluster rectangle.
- For fan-in, all source groups or source nodes are treated as one cluster rectangle.
- Dividers are placed on a side of the cluster rectangle instead of being forced into the gap between endpoint groups.
- North/south cluster sides use horizontal dividers; east/west cluster sides use vertical dividers.
- If a cluster already has a divider on one side, later dividers for the same cluster must choose another side before reusing that side.
...
- hardValid is false when there are node overlaps, group overlaps, edge-node hits, illegal segment overlaps outside valid divider-owned trunk routing, edge identity violations, invalid dividers, or routing fallbacks.
...
- Only engine-generated, valid fan-in/fan-out dividers with more than four edges may create shared trunk routing.
- The divider exemption is semantic: divider-owned trunk segments generated by valid divider routing are exempt; divider leaf overlaps and invalid divider overlaps are illegal.
